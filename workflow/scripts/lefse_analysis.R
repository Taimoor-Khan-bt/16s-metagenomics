...
if (length(args) < 5) stop("Usage: lefse_analysis.R <table_tsv> <taxonomy_tsv> <metadata> <group_col> <out_dir>")
...
if (requireNamespace("microbiomeMarker", quietly = TRUE)) {
  ...
  if (!is.null(mm) && !is.null(marker_table(mm)) && nrow(marker_table(mm)) > 0) {
    ...
    print(plot_ef_bar(mm) + theme_bw() + labs(title = "LEfSe Analysis"))
    dev.off()
...
top_sig <- head(res_df[res_df$p_adj < 0.1, ], 20)
if(nrow(top_sig) > 0) {
  print(ggplot(top_sig, aes(x = reorder(taxon, -p_adj), y = -log10(p_adj))) +
